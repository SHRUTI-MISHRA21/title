# Run this command in terminal before executing the script:
# pip install PyMuPDF
import fitz  # PyMuPDF
import re
import logging
...
class PDFAnalyzer:
    ...
    def _extract_title(self, text_blocks: List[Dict], font_stats: Dict) -> str:
        ...
        # --- START OF CHANGE ---
        # Find the absolute largest font size on the first page. This is our strongest clue.
        max_size_on_page = 0
        ...
        for i, block in enumerate(first_page_blocks):
            ...
            # Calculate the base score using your existing semantic logic
            score = self._calculate_semantic_title_score(block, font_stats, first_page_blocks, i)
            
            # --- START OF CHANGE ---
            # Add a significant bonus if the block uses the largest font.
            # This ensures font size is the dominant factor for the title.
            if max_size_on_page > 0 and abs(block["size"] - max_size_on_page) < 1:
                score += 10  # A strong weight to prioritize this block as the title
            # --- END OF CHANGE ---
...
#     # Replace 'your_document.pdf' with the actual path to your PDF
#     pdf_file_path = r'C:\Users\shrut\challenge 1a\dataset\file02.pdf' 

#     if not os.path.exists(pdf_file_path):
#         logging.error(f"Error: PDF file not found at '{pdf_file_path}'. Please ensure the file exists.")
#         print(f"Error: PDF file not found at '{pdf_file_path}'.")
#         print("Please replace 'your_document.pdf' with the correct path to your PDF file.")
#         return

#     try:
#         analysis_result = analyzer.analyze_pdf(pdf_file_path)

#         if analysis_result:
#             print("\n--- PDF Analysis Result ---")
#             print(f"Title: {analysis_result.get('title', 'N/A')}")
#             print("\nOutline (Headings):")
#             if analysis_result["outline"]:
#                 for heading in analysis_result["outline"]:
#                     print(f"  {heading['level']}: {heading['text']} (Page: {heading['page'] })") # +1 for 1-based page number
#             else:
#                 print("  No headings found.")
#         else:
#             print("PDF analysis failed or returned no results.")

#     except ValueError as ve:
#         logging.error(f"Analysis Error: {ve}")
#         print(f"Analysis Error: {ve}")
#     except Exception as e:
